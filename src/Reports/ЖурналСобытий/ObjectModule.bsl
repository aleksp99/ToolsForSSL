...
Функция ПреобразованиеОтбора(Настройки, Раздел)
	...
	ПараметрКД = Настройки.ПараметрыДанных.Элементы.Найти(Раздел + ".Параметры");
	Если ПараметрКД = Неопределено Или Не (ПараметрКД.Использование И ТипЗнч(ПараметрКД.Значение) = Тип(
		"ХранилищеЗначения")) Тогда
		Возврат Неопределено;
	КонецЕсли;
	Параметры = ПараметрКД.Значение.Получить();
