...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтбираемыеПараметры = Новый ФиксированныйМассив(СтрРазделить(Параметры.ОтбираемыеПараметры, "_"));
	РежимВыбора = Параметры.РежимВыбора;
	
	УстановитьПараметрыРедактора(РедактируемыйСписок);
	
	Если Параметры.РежимВыбора Тогда
		Элементы.УстановитьВсе.Видимость = Ложь;
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если РежимВыбора Тогда
		...
		ЗаполнитьЗначенияСвойств(Результат, Список.НайтиПоИдентификатору(ВыбраннаяСтрока));
		Закрыть(Результат);
	КонецЕсли;
...
Процедура ВыбратьСоставОтбора(Команда)
	...
	Закрыть(ПолучитьОтредактированныйСписок());	
...
&НаСервере
Процедура УстановитьПараметрыРедактора(РедактируемыйСписок)
	
	Если ОтбираемыеПараметры[0] = "ЖурналРегистрации" Тогда
		СтруктураПараметровОтбора = ПолучитьЗначенияОтбораЖурналаРегистрации(ОтбираемыеПараметры[1]);
		ЗначенияОтбора = СтруктураПараметровОтбора[ОтбираемыеПараметры[1]];
	Иначе // ФоновыеЗадания.РегламентноеЗадание
		ЗначенияОтбора = Новый Соответствие;
		Для Каждого Элемент Из РегламентныеЗадания.ПолучитьРегламентныеЗадания() Цикл
			ЗначенияОтбора.Вставить(Элемент.УникальныйИдентификатор, ?(ПустаяСтрока(Элемент.Наименование), ?(
				ПустаяСтрока(
				Элемент.Метаданные.Синоним), Элемент.Метаданные.Имя, Элемент.Метаданные.Синоним), Элемент.Наименование));
		КонецЦикла;
	КонецЕсли;
	// Получение списка представлений событий.
	Если ОтбираемыеПараметры[1] = "Событие" Или ОтбираемыеПараметры[1] = "Event" Тогда
	...
	ИначеЕсли ТипЗнч(ЗначенияОтбора) = Тип("Соответствие") Тогда
		...
		Если ОтбираемыеПараметры[1] = "Событие"
			Или ОтбираемыеПараметры[1] = "Event"
			Или ОтбираемыеПараметры[1] = "Метаданные"
			Или ОтбираемыеПараметры[1] = "Metadata" Тогда
		...
		Иначе 
			...
			Для Каждого ЭлементСоответствия Из ЗначенияОтбора Цикл
				...
				Если (ОтбираемыеПараметры[1] = "Пользователь" Или ОтбираемыеПараметры[1] = "User") Тогда
...
Функция ПолучитьВетвьДерева(Представление, Рекурсия = Ложь)
	...
	Если (ОтбираемыеПараметры[1] = "Метаданные"
			Или ОтбираемыеПараметры[1] = "Metadata")
