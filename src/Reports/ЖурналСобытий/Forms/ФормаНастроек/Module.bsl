...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки)
	...
	Для Каждого Раздел Из СтрРазделить("ЖурналРегистрации,ФоновыеЗадания", ",") Цикл
		...
		Отбор = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НовыеНастройки.ДополнительныеСвойства, Раздел
...
Процедура ЛокальныеПриИзменении(Элемент)
	...
	Раздел = Лев(Элемент.Имя, СтрНайти(Элемент.Имя, "_") - 1);
	Для Каждого Элемент Из Элементы["Страница" + Раздел].ПодчиненныеЭлементы Цикл
		Если Не Элемент.Имя = Раздел + "_ГруппаЛокальные" Тогда
			Элемент.Доступность = ЭтаФорма[Раздел + "_Локальные"];
		КонецЕсли;
	КонецЦикла;
...
Процедура СвойстваНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.")-1) + ".Форма.РедакторСоставаСвойства",
...
Процедура СохранитьЭлементыВнешнегоНабораДанных()
	...
	ДополнительныеСвойства.Вставить("ЖурналРегистрации_Параметры", Отбор);
	...
	ДополнительныеСвойства.Вставить("ФоновыеЗадания_Параметры", Отбор);
